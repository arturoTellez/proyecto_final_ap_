...
parser.add_argument("--num_classes",
                    ...
                    labels to one hot encoding", type=int, default=6)
...
root_path = 'D:/jorge/Documents/ISPRS/Potsdam/trainNPY'
# Load images
img_train_path = 'top_potsdam_2_10_RGB.npy'
...
# Load reference
root_path='D:/jorge/Documents/ISPRS/Potsdam/labelNPY'
img_train_ref_path = 'top_potsdam_2_10_label.npy'
...
print('saving images...')
folder_path = f'D:/jorge/Documents/ISPRS/Potsdam/patch_size={args.patch_size}_' + \
...
for i in tqdm(range(len(patches_tr))):
    ...
    for j in range(len(img_aug)):
        ...
        np.save(os.path.join(folder_path, 'train', filename(i*6 + j)),
                img_normalized)
        # All multitasking labels are saved in one-hot
        # Segmentation
        np.save(os.path.join(folder_path, 'labels/seg', filename(i*6 + j)),
                label_aug_h[j].astype(np.float32))
        # Boundary
        bound_label_h = get_boundary_label(label_aug_h[j]).astype(np.float32)
        np.save(os.path.join(folder_path, 'labels/bound', filename(i*6 + j)),
                bound_label_h)
        # Distance
        dist_label_h = get_distance_label(label_aug_h[j]).astype(np.float32)
        np.save(os.path.join(folder_path, 'labels/dist', filename(i*6 + j)),
                dist_label_h)
        # Color
        ...
        np.save(os.path.join(folder_path, 'labels/color', filename(i*6 + j)),
                hsv_patch)
